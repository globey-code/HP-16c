...
def setup_ui(root, config, custom_font):
    ...
    disp = Display(master=root, x=config.get("display_x", config["margin"]), y=config.get("display_y", config["margin"]),
                   width=config["display_width"], height=config["display_height"], font=custom_font)
    disp.set_mode("DEC")
    disp.update()
    ...
    bezel_frame.place(x=(1049 + 2 * config["margin"] - total_grid_width) / 2, y=config.get("display_y", config["margin"]) + config["display_height"] + config["margin"],
                      width=total_grid_width, height=total_grid_height)
    ...
    for cfg in BUTTONS_CONFIG:
        ...
        btn_dict["frame"].grid(row=cfg["row"], column=cfg["col"], rowspan=rowspan, padx=(25, 2), pady=(20, 2), sticky="nsew")
    ...
    branding_label = tk.Label(bezel_frame, text=" H E W L E T T - P A C K A R D   E M U L A T O R ",
                              font=("Courier", 14, "bold"), bg="#1e1818", fg="#A9A9A9")
...
def create_single_button(root, cfg):
    ...
    frame = tk.Frame(root, bg=cfg.get("bg", "#1e1818"), relief="flat", highlightthickness=1,
                     highlightbackground="white", width=cfg["width"], height=cfg["height"])
